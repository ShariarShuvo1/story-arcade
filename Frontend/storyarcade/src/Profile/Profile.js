    ...
    <div className="profile">
      ...
      {isOwner && (
        <>
          ...
          <button onClick={() => alert('Change Email')}>Change Email</button>
          <button onClick={() => alert('Upload Picture')}>Upload Picture</button>
          <button onClick={() => alert('Delete Profile')}>Delete Profile</button>
        </>
      )}
    </div>
  );
